...
class PerformanceAnalyzer:
    ...
    def __init__(self, url, auth_endpoints: AuthEndpoints, resource_endpoints: ResourceEndpoints, email, password,
                 iterations: int):
        self.url = url
    ...
    def _login(self):
        ...
        response = requests.post(self._build_endpoint_url(self.auth_endpoints.LOGIN), headers=headers, data=payload)
    ...
    def _logout(self):
        response = requests.post(self._build_endpoint_url(self.auth_endpoints.LOGOUT), headers=self._get_auth_headers())
        self._ensure_request_success(response)

    def _test_request(self, endpoint: ResourceEndpoints):
        response = requests.get(self._build_endpoint_url(endpoint), headers=self._get_auth_headers())
        self._ensure_request_success(response)
    ...
    def _create_graphs(self, filename):
        endpoints = [endpoint['name'] for endpoint in self.performance_data['iterations'][0]['endpoints_times']]
        ...
        for i in range(endpoints_count):
            for j in range(len(self.performance_data['iterations'])):
                endpoints_times[i].append(self.performance_data['iterations'][j]['endpoints_times'][i]['time'])
    ...
    def run(self):
        ...
        for iteration in range(self.iterations):
            ...
            self.performance_data['iterations'].append({
                'endpoints_times': endpoints_times,
...
def main():
    performance_analyzer = PerformanceAnalyzer(URL, AuthEndpoints, ResourceEndpoints, EMAIL, PASSWORD, 1)
