...
from fastapi import APIRouter, BackgroundTasks, Depends, Form, HTTPException, Header, status
...
from ..email_manager import create_email_request, create_email_verification_email, send_email
from ..exceptions import CooldownHTTPException, InvalidEnumerationMemberHTTPException, InvalidGrantTypeException
...
def create_user(user: CreateUser,
    ...
    password = models.Password(password_hash=hashed_password, user_id=new_user.id, current=True)
    db.add(password)
    db.commit()
...
def get_users(db: Session = Depends(get_db),
              _=Depends(oauth2.get_admin)) -> dict[str, ReturnUser]:
...
def update_user_details(name: str = Form(Required),
...
def get_user_by_uuid(uuid: UUID4,
                     db: Session = Depends(get_db),
                     _=Depends(oauth2.get_admin)) -> ReturnUserDetailed:
...
def promote_user(uuid: UUID4,
    ...
    if not user.verified:
        ...
        raise HTTPException(detail='Cannot promote an unverified user',
                            status_code=status.HTTP_409_CONFLICT)

    if 'admin' in user.permission_level:
        raise HTTPException(detail='This user already has the highest possible permission level',
                            status_code=status.HTTP_409_CONFLICT)
...
def demote_user(uuid: UUID4,
                db: Session = Depends(get_db),
                _=Depends(oauth2.get_superuser)):
    user = get_user_from_db(uuid=uuid, db=db)

    if 'admin' not in user.permission_level:
        raise HTTPException(detail='This user already has the lowest possible permission level',
                            status_code=status.HTTP_409_CONFLICT)
...
def ban_user(uuid: UUID4,
    ...
    if 'admin' in user.permission_level:
        raise HTTPException(detail='Cannot ban user with elevated permissions level, if you are a superuser and really'
...
def unban_user(uuid: UUID4,
    ...
    if not user.disabled:
        raise HTTPException(detail='This user is not currently banned',
                            status_code=status.HTTP_409_CONFLICT)
