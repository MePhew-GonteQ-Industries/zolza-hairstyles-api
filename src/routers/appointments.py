...
@router.get("/mine")
def get_your_appointments(db: Session = Depends(get_db), user=Depends(oauth2.get_user)):
    appointments_db = (
...
def get_your_appointment(
    db: Session = Depends(get_db), user=Depends(oauth2.get_verified_user)
):
...
@router.get("/all")
def get_all_appointments(db: Session = Depends(get_db), user=Depends(oauth2.get_admin)):
    raise NotImplementedError


@router.get("/any/{id}")
async def get_any_appointment(
    db: Session = Depends(get_db), user=Depends(oauth2.get_admin)
):
    raise NotImplementedError
