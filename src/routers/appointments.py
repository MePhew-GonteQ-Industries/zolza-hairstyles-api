...
def get_appointment_slots(
    ...
    first_available_time = datetime.datetime.now(COMPANY_TIMEZONE) + timedelta(hours=1)
...
def get_all_appointments(
    ...
    if not include_archival:
        appointments_db = appointments_db.join(
            models.AppointmentSlot,
            and_(models.Appointment.end_slot_id == models.AppointmentSlot.id),
        ).filter(models.AppointmentSlot.end_time > datetime.datetime.now(COMPANY_TIMEZONE))
    ...
    appointments_db = appointments_db.order_by(models.Appointment.created_at.desc())
...
def reserve_slots(
    ...
    slots_db = (
        ...
        .where(
            models.AppointmentSlot.start_time
            > datetime.datetime.now().astimezone(COMPANY_TIMEZONE)
        )
        .all()
    )
