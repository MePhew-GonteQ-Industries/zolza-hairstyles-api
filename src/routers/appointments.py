...
def get_all_appointments(
    ...
    if not include_archival:
        appointments_db = appointments_db.join(models.AppointmentSlot, and_(
            models.Appointment.end_slot_id == models.AppointmentSlot.id
        )).filter(
            models.AppointmentSlot.end_time > datetime.datetime.now(PL_TIMEZONE)
        )
