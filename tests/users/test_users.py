...
@pytest.mark.parametrize('content_language, preferred_theme, gender',
                         [
                             ('pl', 'dark', 'male'),
                             ('en', 'light', 'female'),
                             ('pl', 'dark', 'other')
...
def test_create_user(client, content_language, preferred_theme, gender):
    ...
    res = client.post(settings.BASE_URL + ROUTE_PREFIX + 'register',
    ...
    user_data['settings'] = [{'current_value': preferred_theme,
                              'default_value': None,
                              'name': 'preferred_theme'},
                             {'current_value': content_language,
                              'default_value': None,
                              'name': 'language'}]
...
def test_request_email_verification(client):
    ...
    res = client.post(settings.BASE_URL + ROUTE_PREFIX + 'request-email-verification',
                      json=user_email)
...
@pytest.mark.parametrize('verification_token, status_code', [
    (''.join([random.choice(string.ascii_letters) for _ in range(40)]), status.HTTP_401_UNAUTHORIZED)
])
def test_verify_email(client, verification_token, status_code):
    res = client.put(settings.BASE_URL + ROUTE_PREFIX + 'verify-email',
